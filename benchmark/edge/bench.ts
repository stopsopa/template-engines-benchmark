/**
 * PURPOSE:
 * This script benchmarks the Edge.js template engine's performance when executing a cached 
 * template function 1,000,000 times. It tracks total execution time, throughput (ops/sec), and peak memory usage (RSS).
 * 
 * USAGE:
 * 1. Run the benchmark:
 *    NODE_OPTIONS="" node benchmark/edge/bench.ts | tee benchmark/edge/bench.log
 *    Expected Result: Prints performance statistics after completing 1 million executions.
 * 
 * 2. Run in test mode:
 *    NODE_OPTIONS="" node benchmark/edge/bench.ts --test
 ...
 * 
 * 3. Type-checking:
 *    node node_modules/.bin/tsc --noEmit --esModuleInterop benchmark/edge/bench.ts
 * 
 * To compare output:
 * 
 NODE_OPTIONS="" node benchmark/eta/bench.ts --test > benchmark/eta/test.log
 NODE_OPTIONS="" node benchmark/edge/bench.ts --test > benchmark/edge/test.log
 diff benchmark/eta/test.log benchmark/edge/test.log
 */
...
(async function () {
  ...
  edge.mount(import.meta.dirname);

  if (testMode) {
    const result = await edge.render("template.edge", { it: data });
  ...
  for (let i = 0; i < count; i++) {
    await edge.render("template.edge", { it: data });
