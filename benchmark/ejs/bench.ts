/**
 * PURPOSE:
 * This script benchmarks the EJS template engine's performance when executing a cached 
 * template file 1,000,000 times. It tracks total execution time, throughput (ops/sec), and peak memory usage (RSS).
 * 
 * USAGE:
 * 1. Run the benchmark:
 *    NODE_OPTIONS="" node benchmark/ejs/bench.ts | tee benchmark/ejs/bench.log
 *    Expected Result: Prints performance statistics after completing 1 million executions.
 * 
 * 2. Run in test mode:
 *    NODE_OPTIONS="" node benchmark/ejs/bench.ts --test
 ...
 * 
 * 3. Type-checking:
 *    node node_modules/.bin/tsc --noEmit --esModuleInterop benchmark/ejs/bench.ts
 * 
 * To compare output:
 * 
 NODE_OPTIONS="" node benchmark/eta/bench.ts --test > benchmark/eta/test.log
 NODE_OPTIONS="" node benchmark/ejs/bench.ts --test > benchmark/ejs/test.log
 diff benchmark/eta/test.log benchmark/ejs/test.log
 */
import ejs from "ejs";
...
(async function () {
  ...
  const options = {
    ...
    localsName: "it",
    _with: false, // Disable 'with' for better performance and to match 'it.' access
  };

  if (testMode) {
    // @ts-ignore - EJS types might not reflect the Promise return in all versions
    const result = await ejs.renderFile(templatePath, data, options);
  ...
  for (let i = 0; i < count; i++) {
    // @ts-ignore
    await ejs.renderFile(templatePath, data, options);
